...
import sys2_photo_backfill_trial as trial
...
FULL_AUTHORIZATION = (
    "PR #70 owner comment 2026-08-18T09:57:03Z: "
    "TRIAL_AUDIT_PASSED -> FULL_APPEND_AND_OBSIDIAN; reuse 10 audited TRIAL photos "
    "and collect the remaining 648 official detail URLs"
)
...
def validate_status_flicker_probe(
    ...
    normalized_url, path_kind = trial.page_referenced_photo_url(photo_url, source)
    ...
    if captured_round not in range(1, FLICKER_PROBE_ROUNDS + 1):
        raise RuntimeError("状态闪烁聚合 5 轮均未当场采得照片")
...
def run_status_flicker_probe(
    ...
    *,
    ...
    if not trial.detail_id(source) or not normalized_url:
        raise RuntimeError("状态闪烁聚合来源或照片 URL 不在 Issue #69 授权范围")
    matching_rows = [
        row
    ...
    if len(matching_rows) != 1:
        raise RuntimeError("状态闪烁聚合来源不在 658 行固定范围内且唯一")
    row = matching_rows[0]
    ...
    for round_number in range(1, FLICKER_PROBE_ROUNDS + 1):
        ...
        try:
            ...
            state, reference = trial.extract_portrait_reference(html, source, name)
    ...
    if captured_content is None:
        raise RuntimeError(
            "[FATAL - HUMAN_INTERVENTION_REQUIRED] 状态闪烁资源 5 轮均未当场采得照片"
        )
    payload = {
        "meta": {
            "issue": ISSUE_NUMBER,
    ...
    with tempfile.TemporaryDirectory(
        prefix="issue69_flicker_probe_", dir=WORK_DIR
    ) as temporary:
    ...
    for sheet_row, (before, after) in enumerate(
        ...
        for column in trial.BASE_HEADERS:
            ...
            if source not in target_sources:
                raise RuntimeError(f"发现 Issue #69 范围外行修改：{source} {column}")
...
def target_profile_paths(
    ...
    if (
        ...
        raise RuntimeError("FULL 前 658 个来源与 658 份画像不是一一对应")
    return result
...
def apply_file_map(file_map: dict[Path, Path]) -> None:
    for target, source in file_map.items():
        ...
        staging = target.with_name(f".{target.name}.issue69.tmp")
        if staging.exists():
            staging.unlink()
...
def restore_file_targets(backups: dict[Path, Path | None]) -> None:
    for target, backup in backups.items():
        staging = target.with_name(f".{target.name}.issue69.restore")
        if staging.exists():
            staging.unlink()
        if backup is None:
...
- 复用已审计 TRIAL：{meta['trial_reused_count']}；FULL 新抓取：{meta['fresh_downloaded_count']}；其余待抓取目标：{meta['fresh_target_count']}。
...
def validate_full_payload(
    ...
    if int(meta.get("fresh_downloaded_count") or 0) + failed != EXPECTED_FRESH_COUNT:
        raise RuntimeError("FULL 648 个新抓取目标未闭合")
    ...
    for item in reconciliation:
        ...
        if status == "实采":
            ...
            normalized_url, kind = trial.page_referenced_photo_url(
...
def validate_full_installation(payload: dict[str, Any]) -> None:
    ...
    with FULL_CSV_PATH.open("r", encoding="utf-8-sig", newline="") as handle:
        if len(list(csv.DictReader(handle))) != EXPECTED_SCOPE_COUNT:
            raise RuntimeError("FULL 照片对账 CSV 不是 658 行")
...
def run_full(run_date: str) -> dict[str, Any]:
    ...
    if len(target_sources) != EXPECTED_SCOPE_COUNT:
        raise RuntimeError("FULL 固定范围来源链接不是 658 个唯一官网详情 URL")
    flicker_probe_payload: dict[str, Any] | None = None
    if FLICKER_PROBE_JSON_PATH.exists() or FLICKER_PROBE_PHOTO_PATH.exists():
        if not FLICKER_PROBE_JSON_PATH.is_file() or not FLICKER_PROBE_PHOTO_PATH.is_file():
            raise RuntimeError("状态闪烁聚合工件不成对")
        ...
        if probe_source not in target_sources:
            raise RuntimeError("状态闪烁聚合来源不在 FULL 658 行固定范围")
    profile_paths = target_profile_paths(PROFILE_DIR, target_sources)
    ...
    with tempfile.TemporaryDirectory(prefix="issue69_full_", dir=WORK_DIR) as temporary:
        ...
        def add_success(
            ...
            photo_url, path_kind = trial.page_referenced_photo_url(
        ...
        fresh_rows = [
            ...
            if trial.clean_text(row.get("来源链接")) not in seed_sources
        ]
        if len(fresh_rows) != EXPECTED_FRESH_COUNT:
            raise RuntimeError("FULL 待新抓取范围不是 648 行")
        ...
        for index, row in enumerate(fresh_rows, start=1):
            ...
            if detail is None or detail.status != 200 or detail.content_type != "text/html":
                record_failure(
                    row,
                    "详情不可达",
                    f"详情不可达：{attempt_evidence(detail_attempts)}",
                    detail_attempts,
                )
                continue
            html = detail.content.decode(detail.charset, errors="replace")
            state, reference = trial.extract_portrait_reference(html, source, name)
            if state:
                record_failure(row, state, state, detail_attempts)
                continue
            if reference is None:
                raise RuntimeError(f"医生照片引用状态为空：{source}")
            ...
            if photo is None or photo.status != 200 or not photo.content_type.startswith("image/"):
                record_failure(
                    row,
                    "详情不可达",
                    f"照片资源不可达：{attempt_evidence(photo_attempts)}",
                    photo_attempts,
                )
                continue
            if trial.comparable_host(photo.final_url) != trial.OFFICIAL_HOST:
            ...
            if placeholder:
                record_failure(row, "占位图", placeholder, photo_attempts)
                continue
            add_success(
                row,
                {
        ...
        if set(result_by_source) != target_sources:
            raise RuntimeError("FULL 658 行结果来源集合未闭合")
        result_rows = [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Issue #69 孙逸仙纪念医院照片补录 FULL")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--full", action="store_true", help="执行 658 行 FULL 事务")
