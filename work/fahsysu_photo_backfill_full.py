...
import fahsysu_photo_backfill_trial as trial
...
EXPECTED_TRIAL_FAILURE_REUSE_COUNT = trial.EXPECTED_FAILURE_EVIDENCE_COUNT
EXPECTED_FRESH_COUNT = (
    EXPECTED_SCOPE_COUNT
    - EXPECTED_TRIAL_REUSE_COUNT
    - EXPECTED_TRIAL_FAILURE_REUSE_COUNT
)
...
FULL_WARNING_BY_STATE["照片资源不可达"] = (
    "官网本人职业照补录失败：照片资源不可达（详情页 HTTP 200、"
    "focal_point_480 引用数=1、资源 404 悬空引用）"
)
FULL_ALLOWED_ROW_COLUMNS = {"照片链接", "照片文件", "异常提示"}
FULL_AUTHORIZATION = (
    "PR #72 owner comment 2026-08-18T12:42:33Z: "
    "TRIAL_AUDIT_PASSED -> FULL_APPEND_AND_OBSIDIAN; reuse 10 audited TRIAL photos "
    "and 2 audited no-photo-container evidence rows; collect the remaining 848 "
    "official detail URLs. PR #72 owner ruling 2026-08-18T13:21:37Z: dangling "
    "focal reference for Yang Song is fixed as 照片资源不可达 and FULL continues"
)
...
def normalized_photo_reference(photo_url: str, source_link: str) -> tuple[str, str]:
    normalized, path_kind, _itok = trial.page_referenced_photo_url(
        photo_url, source_link
    )
    return normalized, path_kind
...
def downloaded_placeholder_reason(
    ...
    if extension != "gif" or len(content) >= SMALL_GIF_PLACEHOLDER_BYTES:
        return ""
    path = trial.unquote(trial.urlparse(photo_url).path).lower()
...
def validate_status_flicker_probe(
    ...
    if trial.magic_extension(content, media_type) != extension:
        raise RuntimeError("状态闪烁聚合冻结照片魔数不一致")
    if trial.image_dimensions(content) != (
...
def run_status_flicker_probe(
    ...
    photo_url: str,
    *,
    session_factory: Callable[[], trial.OfficialSession] = trial.OfficialSession,
    ...
    if not trial.detail_id(source) or not normalized_url:
        raise RuntimeError("状态闪烁聚合来源或照片 URL 不在 Issue #71 授权范围")
    matching_rows = [
        row
    ...
    if len(matching_rows) != 1:
        raise RuntimeError("状态闪烁聚合来源不在 860 行固定范围内且唯一")
    row = matching_rows[0]
    ...
    for round_number in range(1, FLICKER_PROBE_ROUNDS + 1):
        ...
        try:
            ...
            reference = trial.analyze_doctor_media(html, source, name)
    ...
    with tempfile.TemporaryDirectory(
        prefix="issue71_flicker_probe_", dir=WORK_DIR
    ) as temporary:
    ...
    for sheet_row, (before, after) in enumerate(
        ...
        for column in trial.BASE_HEADERS:
            ...
            if source not in target_sources:
                raise RuntimeError(f"发现 Issue #71 范围外行修改：{source} {column}")
...
def target_profile_paths(
    ...
    if (
        ...
        raise RuntimeError("FULL 前 860 个来源与 860 份画像不是一一对应")
    return result
...
def apply_file_map(file_map: dict[Path, Path]) -> None:
    for target, source in file_map.items():
        ...
        staging = target.with_name(f".{target.name}.issue71.tmp")
        if staging.exists():
            staging.unlink()
...
def restore_file_targets(backups: dict[Path, Path | None]) -> None:
    for target, backup in backups.items():
        staging = target.with_name(f".{target.name}.issue71.restore")
        if staging.exists():
            staging.unlink()
        if backup is None:
...
| {meta['expected_count']} | {meta['downloaded_count']} | {meta['failed_count']} | {meta['disk_photo_count']} | {meta['blank_count']} |
    ...
    - 复用已审计 TRIAL 照片：{meta['trial_reused_count']}；复用已审计失败证据：{meta['trial_failure_reused_count']}；FULL 新抓取成功：{meta['fresh_downloaded_count']}；FULL 新抓取失败：{meta['fresh_failed_count']}；新抓取目标：{meta['fresh_target_count']}。
- 失败率：{meta['failure_ratio']:.2%}；状态闪烁：{meta['status_flicker_count']}；已完成 5 轮聚合并复用冻结原图：{meta['status_flicker_resolved_count']}。
...
- 路由：{json.dumps(meta['detail_template_counts'], ensure_ascii=False)}；原图路径：{json.dumps(meta['path_kind_counts'], ensure_ascii=False)}。
...
def validate_full_payload(
    ...
    if (
        ...
        raise RuntimeError("FULL 848 个新抓取目标未闭合")
    ...
    for item in reconciliation:
        ...
        state = trial.clean_text(item.get("失败三态"))
        if status == "实采":
            ...
            if "focal_point_480_reference_count" not in evidence:
...
def validate_full_installation(payload: dict[str, Any]) -> None:
    ...
    with FULL_CSV_PATH.open("r", encoding="utf-8-sig", newline="") as handle:
        if len(list(csv.DictReader(handle))) != EXPECTED_SCOPE_COUNT:
            raise RuntimeError("FULL 照片对账 CSV 不是 860 行")
...
def run_full(run_date: str) -> dict[str, Any]:
    ...
    if len(target_sources) != EXPECTED_SCOPE_COUNT:
        raise RuntimeError("FULL 固定范围来源链接不是 860 个唯一官网详情 URL")
    ...
    if FLICKER_PROBE_JSON_PATH.exists() or FLICKER_PROBE_PHOTO_PATH.exists():
        ...
        if probe_source not in target_sources:
            raise RuntimeError("状态闪烁聚合来源不在 FULL 860 行固定范围")
        captured_round = int(
    ...
    seed_samples = trial_payload.get("photo_samples", [])
    ...
    trial_failure_sources = {
        trial.clean_text(item.get("source_link")) for item in trial_failure_records
    }
    if (
        len(trial_failure_records) != EXPECTED_TRIAL_FAILURE_REUSE_COUNT
        or len(trial_failure_sources) != EXPECTED_TRIAL_FAILURE_REUSE_COUNT
        or not trial_failure_sources <= target_sources
        or trial_failure_sources & seed_sources
    ):
        raise RuntimeError("FULL 复用的 2 条 TRIAL 失败证据范围漂移")
    ...
    with tempfile.TemporaryDirectory(prefix="issue71_full_", dir=WORK_DIR) as temporary:
        ...
        trial_failure_by_source = {
            trial.clean_text(item.get("source_link")): item
            for item in trial_failure_records
        }
        for source in sorted(trial_failure_sources):
            item = trial_failure_by_source[source]
            evidence_detail = {
                "observed_utc": trial.clean_text(item.get("detail_probe_utc")),
                "detail_http": item.get("detail_status"),
                "focal_point_480_reference_count": item.get(
                    "focal_point_480_reference_count"
                ),
                "resource_urls": list(item.get("excluded_resource_urls") or [source]),
                "excluded_resources": list(item.get("excluded_resources") or []),
                "detection_feature": trial.clean_text(item.get("detection_feature")),
                "template_signature": trial.clean_text(item.get("template_signature")),
            }
            record_failure(
                rows_by_source[source],
                trial.clean_text(item.get("failure_state")),
                evidence_detail,
                list(item.get("detail_attempts") or []),
                "TRIAL_FAILURE_REUSE",
            )
        ...
        if len(fresh_rows) != EXPECTED_FRESH_COUNT:
            raise RuntimeError("FULL 待新抓取范围不是 848 行")
        ...
        for index, row in enumerate(fresh_rows, start=1):
                    ...
                    record_failure(
                        ...
                        "照片资源不可达",
                        {
                            ...
                            "detection_feature": (
                                ...
                                "focal_point_480；执行时点 3 次尝试后，5 轮聚合"
                                "均未取得照片；owner 复测资源 HTTP 404"
                            ),
                            "template_signature": (
                            ...
                            "owner_ruling": (
                                "https://github.com/nancywrayg57-jpg/"
                                "doctor-data-collection/pull/72#issuecomment-5328766700"
                            ),
                        },
                        rounds,
                        FLICKER_PROBE_ORIGIN,
            ...
            if detail is None or detail.status != 200 or detail.content_type != "text/html":
                evidence_detail = {
                    ...
                    "focal_point_480_reference_count": None,
            ...
            if photo is None or photo.status != 200 or not photo.content_type.startswith("image/"):
                evidence_detail = {
                    ...
                    "excluded_resources": [],
                    "detection_feature": "页面引用唯一 focal_point_480；照片资源不可达；"
        ...
        if set(result_by_source) != target_sources:
            raise RuntimeError("FULL 860 行结果来源集合未闭合")
        result_rows = [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Issue #71 中山大学附属第一医院照片补录 FULL")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--full", action="store_true", help="执行 860 行 FULL 事务")
