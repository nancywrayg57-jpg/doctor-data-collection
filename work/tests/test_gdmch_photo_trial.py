...
from collect_official_doctors_batch import (  # noqa: E402
    ...
    gdmch_photo_url,
    merge_gdmch_identity_rows,
    parse_gdmch_detail,
    strip_gdmch_schedule_tail,
...
class GdmchPhotoTrialTests(unittest.TestCase):
    ...
    def test_non_doctor_accounts_are_excluded_but_doctor_is_kept(self) -> None:
        ...
        self.assertTrue(gdmch_non_doctor_card("马淑丹", "收费"))
    ...
    def test_full_schedule_variants_are_removed(self) -> None:
        for value in (
            "出诊地点及时间：番禺院区，周二、四、五、日",
            "周一、周二、周四番禺院区，周五越秀院区。",
            "专科门诊：周一全天（越秀）、周五上午（番禺）",
        ):
            cleaned, removed = strip_gdmch_schedule_tail(value)
    ...
    def test_full_identity_merge_keeps_one_verified_duplicate_pair(self) -> None:
        rows = []
        same_names = {
            "郭庆禄": ("34640", "34931"),
            "周真": ("32647", "32821"),
        ...
        for name, detail_ids in same_names.items():
            for index, detail_id in enumerate(detail_ids):
                rows.append(
                    {
                        "姓名": name,
                        "科室_分类页": "乳腺疾病影像学诊断（清远院区）"
                        if name == "郭庆禄"
                        else f"科室{detail_id}",
                        "科室_列表卡片": "",
                        "职称身份原文": "主任医师",
                        ...
                        "异常提示": "",
                        "_gdmch_detail_id": detail_id,
                        "_gdmch_photo_url": (
                            "https://wx.e3861.com/sfyAdmin/Images/Doctor/34640.jpg"
                            if name == "郭庆禄" and index == 0
                            else ""
                        ),
                    }
                )

        merged, reconciliation = merge_gdmch_identity_rows(rows)

        self.assertEqual(len(merged), 7)
        self.assertEqual(len(reconciliation), 7)
        merged_groups = [
            item for item in reconciliation if int(item.get("relation_count") or 0) > 1
        ]
        self.assertEqual(len(merged_groups), 1)
        self.assertEqual(set(merged_groups[0]["detail_ids"]), {"34640", "34931"})
        self.assertEqual(
            {item["name"] for item in reconciliation if item["resolution"] == "同名待甄别"},
            {"周真", "刘颖", "何裕"},
        )

    def test_full_validator_accepts_owner_approved_reconciliation(self) -> None:
        known_same_name_ids = [
            "34640",
            "34931",
            "32647",
        ...
        generic_detail_count = (
            GDMCH_EXPECTED_RELATION_COUNT
            - GDMCH_EXPECTED_NON_DOCTOR_COUNT
            - len(known_same_name_ids)
        )
        detail_ids = known_same_name_ids + [
            str(50000 + index) for index in range(generic_detail_count)
        ]
        identity_groups = [["34640", "34931"]] + [
            [value] for value in detail_ids if value not in {"34640", "34931"}
        ]
        known_names = {
            "34640": "郭庆禄",
            "32647": "周真",
        ...
        for index, group in enumerate(identity_groups):
            detail_id = group[0]
            name = known_names.get(detail_id, f"赵{chr(0x4E00 + index)}")
            source = (
                "https://www.e3861.com/keshizhuanjia/zhuanjiajieshao/"
                f"{detail_id}.html"
            )
            has_photo_source = index < GDMCH_EXPECTED_PHOTO_AVAILABLE_COUNT
            warning = "照片获取失败" if has_photo_source else ""
            rows.append(
                {
                    "姓名": name,
                    "科室_分类页": "内科",
            ...
            identities.append(
                {
                    ...
                    "resolution": "同一人归并" if len(group) > 1 else "唯一身份",
                }
            )
            if has_photo_source:
                photo_errors.append({"source_link": source})
        ...
        payload = {
            "meta": {
                ...
                - GDMCH_EXPECTED_NON_DOCTOR_COUNT,
                "photo_census_available_count": GDMCH_EXPECTED_PHOTO_AVAILABLE_COUNT,
                "photo_census_placeholder_count": (
                    GDMCH_EXPECTED_RELATION_COUNT
                    - GDMCH_EXPECTED_NON_DOCTOR_COUNT
                    - GDMCH_EXPECTED_PHOTO_AVAILABLE_COUNT
                ),
                ...
                "photo_expected_count": GDMCH_EXPECTED_PHOTO_AVAILABLE_COUNT,
                "photo_downloaded_count": 0,
                "photo_failed_count": GDMCH_EXPECTED_PHOTO_AVAILABLE_COUNT,
                "photo_no_source_count": GDMCH_EXPECTED_FINAL_IDENTITY_COUNT
                - GDMCH_EXPECTED_PHOTO_AVAILABLE_COUNT,
                "large_photo_count": 0,
                "photo_policy_status": "OWNER_APPROVED_ORIGINAL_NO_WIDTH_LIMIT",
            },
            ...
            "excluded_candidates": [
                {
                    "detail_id": str(90000 + index),
                    "name": f"测试号{index}",
                    "list_title": "",
                }
                for index in range(GDMCH_EXPECTED_NON_DOCTOR_COUNT)
            ],
            "gdmch_detail_reconciliation": [
                {"detail_id": value} for value in detail_ids
            ],
            "gdmch_identity_reconciliation": identities,
            "photo_samples": [],
            "photo_errors": photo_errors,
        }

        validate_gdmch_full_append(payload)
        with self.assertRaisesRegex(RuntimeError, "eligible_candidate_count"):
            payload["meta"]["eligible_candidate_count"] -= 1
            validate_gdmch_full_append(payload)

    def test_full_validator_requires_payload(self) -> None:
        with self.assertRaisesRegex(RuntimeError, "缺少全量 payload"):
            validate_gdmch_full_append(None)
