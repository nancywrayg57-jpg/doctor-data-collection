...
import smukq_photo_backfill_trial as trial
...
FULL_AUTHORIZATION = (
    "PR #82 owner comment 2026-08-19T08:39:08Z: "
    "TRIAL_AUDIT_PASSED -> FULL_APPEND_AND_OBSIDIAN; fixed scope 95; "
    "reuse 10 audited TRIAL originals; strict img.content_img src and "
    "/Uploads/Upload/ page-reference gates"
)
PULL_REQUEST_NUMBER = 82
REQUEST_MODE = "urllib-browser-ua-get/no-cookie/no-proxy/no-bypass"
TEMPLATE_SIGNATURE = "img.content_img src"
...
class OfficialUrlOpenSession:
    ...
    def get(self, url: str) -> HttpResult:
        request = Request(url, headers={"User-Agent": trial.framework.USER_AGENT})
...
def media_analysis(
    *,
    parser: trial.SmukqPhysicianPageParser,
    state: str,
    photo_url: str = "",
    ...
    return MediaAnalysis(
        page_name=parser.names[0] if len(parser.names) == 1 else "",
        page_title=parser.titles[0] if len(parser.titles) == 1 else "",
        state=state,
        photo_url=photo_url,
        ...
        container_html_snippet=(
            parser.portrait_snippets[0] if len(parser.portrait_snippets) == 1 else ""
        ),
...
def analyze_full_doctor_media(
    ...
    if parser.names != [expected]:
        raise RuntimeError(
            f"详情姓名与底表不一致：底表={expected_name} 官网={parser.names or ['空']} "
            f"{source_link}"
        )
    if len(parser.titles) != 1:
        raise RuntimeError(f"详情职称主结构不唯一：{source_link} 数量={len(parser.titles)}")
    if not parser.portrait_attrs:
        return media_analysis(
            parser=parser,
            state="无照片容器",
            detection_feature="HTTP 200 detail contains no img.content_img",
        )
    if len(parser.portrait_attrs) != 1:
        raise RuntimeError(
            f"img.content_img 容器不唯一：{source_link} 数量={len(parser.portrait_attrs)}"
        )
    raw_url = trial.clean_text(parser.portrait_attrs[0].get("src"))
    if not raw_url:
        return media_analysis(
            parser=parser,
            state="无照片容器",
            detection_feature="unique img.content_img lacks src",
        )
    absolute = urljoin(source_link, raw_url)
    decoded_query = trial.framework.suspicious_query_decoding(absolute)
    ...
    if decoded_query or path_marker:
        ...
        return media_analysis(
            ...
            excluded=({"url": absolute, "reason": "占位图", "feature": feature},),
        )
    normalized = ORIGINAL_TRIAL_PAGE_REFERENCED_PHOTO_URL(raw_url, source_link)
    if not normalized:
    ...
    return media_analysis(
        ...
        photo_reference_count=1,
        detection_feature=(
            "only the unique img.content_img src under /Uploads/Upload/ is eligible; "
            "/Home/images/, /Public/Home/images/ and all other page images are excluded"
        ),
    )
...
def placeholder_response_reason(
    ...
    decoded_query = trial.framework.suspicious_query_decoding(photo_url)
    if decoded_query:
...
def validate_full_page_title(
    ...
    if not actual:
        raise RuntimeError(f"详情 span.content2_span2 为空：{source}")
    if actual != expected:
        TITLE_VARIANCE_EVIDENCE[source] = {
            ...
            "decision": (
                "PHOTO_ONLY_SCOPE: exact name and unique img.content_img passed; "
...
def enforce_repository_relative_payload(payload: dict[str, Any]) -> None:
    ...
    if TITLE_VARIANCE_EVIDENCE:
        ...
        meta["page_title_variance_count"] = len(TITLE_VARIANCE_EVIDENCE)
        meta["page_title_validation_policy"] = (
            "span.content2_span2 must be non-empty; differences are recorded; "
    ...
    if any(Path(item.get("path", "")).is_absolute() for item in payload.get("profile_integrity", [])):
        raise RuntimeError("FULL profile_integrity 不是仓库相对路径")
...
def configure_framework() -> None:
    ...
    compatibility = {
        ...
        "magic_extension": trial.magic_extension,
