...
OFFICIAL_HOME = "https://www.gy3y.cn/index"
DIRECTORY_URL = "https://www.gy3y.cn/kstd/zjjs.html"
OFFICIAL_HOST = "gy3y.cn"
EXPECTED_SCOPE_COUNT = 422
EXPECTED_TRIAL_COUNT = 10
MIN_TRIAL_DEPARTMENTS = 10
...
BASE_HEADERS = [
    "序号", "医院", "姓名", "科室_分类页", "科室_列表卡片", "职称_关键词",
    "职称身份原文", "重点优先级", "重点关注范围", "重点疾病标签",
    "擅长诊疗方向摘录", "亮眼经历线索", "列表简介", "详情正文摘录",
    "来源类型", "来源链接", "照片链接", "照片文件", "采集入口", "采集方式",
    "采集日期", "详情页状态", "已建画像", "异常提示", "复核状态",
]
...
def is_html_document(content_type: str, content: bytes) -> bool:
    if clean_text(content_type).lower() not in {"text/html", "text/plain"}:
        return False
    prefix = content[:8192].lower()
    return bool(re.search(br"<(?:!doctype\s+html|html|head|title|body)\b", prefix))
...
def detail_id(value: Any) -> str:
    ...
    if (
        ...
        return ""
    match = re.fullmatch(
        r"/ks/[A-Za-z0-9_/-]+/doctor_(\d+)\.html", parsed.path
    )
...
def inspect_portrait_reference(
    ...
    parser.feed(html)
    if clean_text(parser.title).casefold() in {"404", "404.html"}:
        return "详情不可达", None
...
def load_scope_rows(*, require_blank_photo_fields: bool = True) -> list[dict[str, Any]]:
    ...
    for expected_name, expected_id in KNOWN_UNREACHABLE_SCOPE:
        matches = [
            row for row in rows if clean_text(row.get("姓名")) == expected_name
        ]
        if len(matches) != 1 or detail_id(matches[0].get("来源链接")) != expected_id:
            raise RuntimeError(
                f"Owner 已知不可达范围证据漂移：{expected_name} doctor_{expected_id}"
            )
    return rows
...
- 页面实际引用路径：`{json.dumps(meta['reference_kind_counts'], ensure_ascii=False)}`；仅取详情页唯一 `div.photo img`，正文叙事图和 floatcard 等非候选区域未进入解析范围。
...
| {EXPECTED_SCOPE_COUNT} 行估算 | {meta['estimated_full_mib']:.2f} MiB |
...
- TRIAL 只写入 `work` 独立工件，未写入口台账、总底表三载体、正式照片目录、{EXPECTED_SCOPE_COUNT} 份画像或索引。
...
def validate_payload(payload: dict[str, Any]) -> None:
    ...
    if meta.get("scope_count") != EXPECTED_SCOPE_COUNT:
        errors.append("范围行数不是 422")
    if meta.get("scope_unique_source_count") != EXPECTED_SCOPE_COUNT:
        errors.append("范围来源链接不是 422 个唯一值")
    if meta.get("scope_blank_photo_row_count") != EXPECTED_SCOPE_COUNT:
        errors.append("TRIAL 基线照片字段不是全空")
    if meta.get("known_unreachable_scope") != [
        {"name": name, "detail_id": detail_id_value}
        for name, detail_id_value in KNOWN_UNREACHABLE_SCOPE
    ]:
        errors.append("Owner 已知不可达范围证据不一致")
    ...
    if (
        set(kinds) - set(PHOTO_PATH_PATTERNS)
        or sum(int(value) for value in kinds.values()) != meta.get("photo_sample_count")
    ):
        errors.append("页面引用路径风格不完整或越界")
...
def run_trial(run_date: str) -> dict[str, Any]:
    ...
    home_status, home_type, _, home_content = session.get(OFFICIAL_HOME)
    if home_status != 200 or not is_html_document(home_type, home_content):
        raise RuntimeError(
            f"官网首页常规会话建立失败：HTTP {home_status} {home_type}"
        )
    directory_status, directory_type, _, directory_content = session.get(
        DIRECTORY_URL, OFFICIAL_HOME
    )
    if directory_status != 200 or not is_html_document(
        directory_type, directory_content
    ):
        raise RuntimeError(
    ...
    payload = {
        "meta": {
            ...
            "home_content_type": home_type,
            "directory_http_status": directory_status,
            "directory_content_type": directory_type,
            "known_unreachable_scope": [
                {"name": name, "detail_id": detail_id_value}
                for name, detail_id_value in KNOWN_UNREACHABLE_SCOPE
            ],
    ...
    for sheet_row, (before, after) in enumerate(
        ...
        for column in BASE_HEADERS:
            ...
            if source not in target_sources:
                raise RuntimeError(f"发现 Issue #65 范围外行修改：{source} {column}")
...
def validate_full_payload(payload: dict[str, Any], photo_root: Path) -> None:
    ...
    if len(reconciliation) != expected or len(rows) != expected or len(photos) != downloaded:
        raise RuntimeError("FULL 422 行对账工件数量不一致")
...
def apply_file_map(file_map: dict[Path, Path]) -> None:
    for target, source in file_map.items():
        ...
        staging = target.with_name(f".{target.name}.issue65.tmp")
        if staging.exists():
            staging.unlink()
...
def restore_file_targets(backups: dict[Path, Path | None]) -> None:
    for target, backup in backups.items():
        staging = target.with_name(f".{target.name}.issue65.restore")
        if staging.exists():
            staging.unlink()
        if backup is None:
...
def target_profile_paths(
    ...
    if len(profile_files) != EXPECTED_PROFILE_COUNT or set(result.values()) != profile_files:
        raise RuntimeError("FULL 前 422 个来源与 422 份画像不是一一对应")
    return result
...
def run_full(run_date: str) -> dict[str, Any]:
    ...
    if len(scope_rows) != EXPECTED_SCOPE_COUNT or len(target_sources) != EXPECTED_SCOPE_COUNT:
        raise RuntimeError("FULL 固定范围不是 422 个唯一官网医生详情来源")
    ...
    with tempfile.TemporaryDirectory(prefix="issue65_full_", dir=WORK_DIR) as temporary:
        ...
        if len(result_rows) != EXPECTED_SCOPE_COUNT:
            raise RuntimeError(f"FULL 结果行不是 422：{len(result_rows)}")
        updated_by_source = {
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument(
        "--full",
        action="store_true",
        help="按 PR #66 Owner 授权执行 422 行照片回填和画像照片块最小刷新",
    )
    mode.add_argument(
        "--validate-full",
        action="store_true",
