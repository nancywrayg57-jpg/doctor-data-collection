# Codex 下一步提示词

> 用途：Claude owner 或管理员写给 Codex developer 的最新可执行提示词。Codex 新会话启动时，在读取 `Agent.md`、路线图、需求文档和架构决策后，必须读取本文件。
> 当前状态：`READY`（TRIAL）。序号 13 已按管理员裁决跳过（PR #10 合并、Issue #9 关闭、台账留痕）；当前任务为 **Issue #11 序号 14 南方医科大学第五附属医院试采**，完整指令见 Issue #11。

## GitHub 身份

- 仓库：`https://github.com/nancywrayg57-jpg/doctor-data-collection.git`
- Codex developer：`xtzhou247`；Claude owner：`nancywrayg57-jpg`
- Codex 不直接推送或合并 `main`，不自行批准 PR；远端写入前确认身份为 `xtzhou247`。

## 当前指令

```text
Status: READY
Phase: TRIAL
LedgerSequence: 14
Hospital: 南方医科大学第五附属医院
City: 广州市
OfficialHomeURL: http://www.ny5y.cn/
DoctorDirectoryURL: http://www.ny5y.cn/zhuanjia_mingyi.php?id=100 http://www.ny5y.cn/zhuanjia_lingnan.php?id=162
ReviewStatus: 确认可采集（owner 预核验：专家风采页直接挂约 134 个 yisheng_xq.php 医生详情链接）
Difficulty: A-优先自动采集
Task: 逐入口普查+跨入口去重后试采 10 位（覆盖尽量多科室），不写入总底表；材料推送 PR 后停止等待 Claude 审计。
GitHubIssue: https://github.com/nancywrayg57-jpg/doctor-data-collection/issues/11
```

执行要点：岭南名医（162）为荣誉栏目，与专家风采去重、荣誉身份归职称/亮眼线索、科室优先真实科室；不纳入科室介绍页与研究生导师简介栏目；仅收 `yisheng_xq.php` 官方详情；站点为 http。

## 流程口径（管理员 2026-08-11/12）

试采审计通过 → FULL_APPEND_AND_OBSIDIAN（追加无中途审计）→ **画像审计** → 通过后合并 PR → 关闭 Issue 后才可领取下一家。同时只允许一个 open 任务 Issue。无法采集（反爬拦截）则熔断回报、直接跳过该院。

## 待管理员事项（留痕）

1. 台账序号 10、12 行入口字段修正（PR #6 / Issue #7 留痕）。
2. 存量 5 家医院「亮眼经历线索」导航污染 343 条，待裁决清理。
3. 序号 13 已标记『跳过-反爬拦截』（PR #10）。

## 合规红线

1. 仅使用医院官网等官方公开渠道。
2. 禁止绕过登录、验证码、反爬或权限限制；禁第三方平台、患者评价、隐私。
3. 官网没有的信息保持空白，不推断、不补造。
4. 不生成疗效承诺、排名、患者评价或无来源亮点。
