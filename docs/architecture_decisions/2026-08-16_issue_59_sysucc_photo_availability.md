...
首次 FULL 工件提交并以非强制 Git Data API 推送到原 PR #60 后，进入 owner 最终画像审计。

## 13. Owner 终审占位图返修

Owner 于 2026-08-16T16:53:45Z 对 head `29879ab` 明确终审“不通过”：六项数据对账与其余 535 张照片均通过，但夏小燕 `/node/3701` 的 `nopic.gif`（GIF87a、28,519 bytes、400×600）实际为灰底禁止符号与“暂无图片”占位图，首次 FULL 误报占位图 0。

根因是原检测只在下载前检查 URL 越界分支中的 placeholder 路径标记，下载成功且路径合法的 GIF 不再做字节/视觉占位判定。最小返修及防复发如下：

1. 新增下载后占位启发式：仅针对小于 40 KiB 的 GIF，检查 `nopic/noimage/placeholder` 等路径标记，并以低色板且浅灰中性像素占比至少 70% 识别“暂无图片”类灰底图；彩色小 GIF 回归样本不误判。
2. 夏小燕底表行照片两列清空，异常提示追加“照片为占位图（暂无图片），留空”；payload/CSV/XLSX 三载体逐值一致。
3. 删除该 GIF；夏小燕画像移除照片嵌入区块，并以重新插入后字节与返修前完全一致证明恢复到插入前状态；其他画像零修改。
4. 更新后四数：应采 542、实采 535、失败 7（详情不可达 3、无照片容器 3、占位图 1）、taxonomy 不适用 1、留空 8；`535 + 7 = 542`、`7 + 1 = 8`、`535 + 8 = 543`。
5. 正式差异调整为 1,078 个单元格：`照片链接` 535、`照片文件` 535、`异常提示` 8。正式照片目录为 535 个文件、94,423,404 bytes（90.05 MiB）。
6. 返修为临时构建→全量验证→备份→原子交换→落盘复核事务；入口台账与总底表更新报告哈希继续保持不变。
7. 返修后 `--validate` 通过；Issue #59 专项 unittest 18/18、完整 unittest 220/220；最终 XLSX 用指定 `@oai/artifact-tool` 导入，目标行渲染正常且公式错误扫描 0。

当前停止点仍为 `FULL_READY_FOR_FINAL_OWNER_AUDIT`，但状态为“已完成 owner 指定的 5 项最小返修，等待复审”。不得自行合并 PR、关闭 Issue 或处理下一 Issue。
...
- 完成 owner 终审唯一占位图返修：夏小燕照片两列留空、删除 GIF、画像恢复插入前状态
- 最终闭环 542+1：实采 535、失败 7、taxonomy 不适用 1、留空 8
- 三载体 543 行 25 列逐值一致；正式差异为照片两列各 535 与异常提示 8
- 535 个既有画像仅新增照片区块；不新建画像、不改索引
- 专项 18/18、完整 220/220、FULL validator、照片字节和 XLSX 视觉验证均通过
Next:
- 提交并以 base_tree 构造返修 tree，通过非强制 Git Data API 更新原分支与 PR #60
- CI 成功后发布 FULL_REPAIR_READY_FOR_OWNER_REAUDIT 信标
- 恢复自动化并等待 owner 对 5 项返修复审
Constraints:
- 只使用详情页自身引用的官方本人职业照
