...
- Python/Node 语法检查通过。
- `python -m unittest discover -s work/tests -p 'test_*.py' -v`：122 项通过；新增 GDGH 专项覆盖严格 URL、真实格式魔数、10 图门禁、同名异常降级和抽样去重。
...
- 解决：将夹具改为 10 个合法中文测试姓名后重跑；122 项全部通过。
...
- 122 项单测、5 表视觉核验、10 图视觉核验和 FULL 负向门禁通过
Next:
